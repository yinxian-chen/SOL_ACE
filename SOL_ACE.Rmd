...
### Primary analysis
  ...
  - Two-level ACEs was associated with increased **DunedinPACE** at the first visit in in two models
  
  - Total ACEs score was associated with increased **GrimAA** and **GrimAage** at the first visit in two models
...
glm.ace.ub<-function(data, exposure, outcome) {
  ...
  formula_1<-as.formula(paste(outcome, "~", exposure,"+ age_change + AGE + 
                              GENDER + CENTER + age_change:", exposure))
  ...
  formula_2<-as.formula(paste(outcome, "~", exposure,"+ age_change + 
                              AGE + GENDER + CENTER + EDUCATION_C2 + age_arr_US 
                              + age_change:", exposure))
...
kable(test.3, format = "latex", booktabs = T, digits = 3, 
      caption = "Sensitivity Analysis by Accounting for Unbalanced Design")%>% 
...
glm.ace.cell<-function(data, exposure, outcome) {
  formula.cell<-as.formula(paste(outcome, "~", exposure,"+ age_change + 
                                 AGE + GENDER + CENTER + EDUCATION_C2 + 
                                 age_arr_US + NK + B + MO + GR + CD4 + 
                                 CD8 + age_change:", exposure))
...
glm.ace.ac<-function(data, exposure, outcome) {
  formula.ac<-as.formula(paste(outcome, "~", exposure,"+ age_change + 
                               AGE + GENDER + CENTER + EDUCATION_C2 + 
                               age_arr_US + EV1 + EV2 + EV3 + EV4 
                               + EV5 + age_change:", exposure))
