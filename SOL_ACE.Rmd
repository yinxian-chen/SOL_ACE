...
#### Covariates
  ...
  - Potential confounders
    ...
    - Interval between two visit (served as the time variable)
  ...
  - Secondary analysis (for potential EMM)
    ...
    - Sex-specific estimate
    
    - Birth-place-specific estimate
...
### Missing data issue
  ...
  - Over 10% of subjects missed the data of parental education
    ...
    - Unlikely to be Missing Completely at Random (MCAR) because most missing value is not due to non-response but that subjects or their parents forgot.
...
### Multiple imputation
  ...
  - All covariates + outcome + additional SES variables (years in the US, current education attainment, income level, and ethnicity background) were involved in the imputation
  
  - To improve computability, only variables have >0.1 correlation with the missing variables itself or missingness of those variables were finally included in predictions.
  
  - To achieve relative efficiency=0.95, five imputed datasets should be generated
  
    - Five replications + ten iterations in each dataset were set up in the imputation
  
  - The imputed plots showed that the convergence is acceptable, meaning that we can move to the next step
  

```{r,echo=F}
predma<-quickpred(sol.ana.imp, exclude = c("ID","WEIGHT_NORM_OVERALL_EPIGEN","age_v1","age_v2"))
...
imp_5<-mice(sol.ana.imp, m=5, maxit = 10, predictorMatrix = predma, method = predmethod, print = F)
...
imp_list_new<-lapply(imp_list, function(data) {
  ...
  data<-data%>%mutate(#age_arr_US = factor(ifelse(US_BORN==0|is.na(US_BORN),ifelse(AGE-YRSUS<18,2,3),1)),
...
#### Adjusted for cell types
  ...
  - Adjusting for cell types yielded a consistent result (also a bit increase in precision)
...
#### Parental separate as the covariate
  ...
  - Pulling out parental separation did not change the result (high consistency)
...
```{r,echo=F}
result.matrix.2$variable<-factor(rep(c(1, 2, 3, 4, 5),3), levels = c(1,2,3,4,5), labels = c("1-3 ACEs", "4+ ACE", "Time", "1-3 ACEs*Time", "4+ ACEs*Time"))#rep(c("4+ ACEs", "Time", "4+ ACEs*Time"),3)
...
#### Assessment for ACE items
  ...
  - All above showed non-negligible effect sizes (though not all of them are statistically significant)
...
ana.plot.specific.1<- ggplot(data=ace_specific_tot[ace_specific_tot$variable=="ACE*Time",],
  ...
  geom_hline(yintercept =0, linetype=2)+
  ylab("Coefficient (95% CI)")+
  labs(title = "Association between Individual ACE Items and Rate of Change in EAA over Follow-up", col = "Individual ACE items")+
  geom_errorbar(aes(ymin=lower, ymax=upper,col=type),width=0.2,cex=0.5) +
  facet_wrap(~outcome,strip.position="left",nrow=3,scales = "free_y")+theme_bw()+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
#### Adjust for ancestry
  ...
  - Association decrease for GrimAA and GrimAge but slightly increase for DunedinPACE
  
  - Precision decrease (sample size drops)
...
#### Complete case analysis
  ...
  - There is evidence that not dealing with missing value may lead to overestimated results.
...
#### EMM by Sex
  ...
  - Difference was observed in sex-specific estimate for EAA at first visit but not in the rate of change in EAA
  
  - However, confidence interval was highly overlapped, which indicates less confidence in recognizing the EMM by sex
...
#### Stratified analysis by birth place

  - Strong evidence showed that birth place modified the association
  ...
  - P-value for interaction was also <0.05 (statistically significant EMM)
