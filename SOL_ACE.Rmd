...
glm.ace.ub<-function(data, exposure, outcome) {
  ...
  formula_1<-as.formula(paste(outcome, "~", exposure,"+ age_change + AGE_correct + GENDER + fa_edu_3c + ma_edu_3c + age_change:", exposure))
...
glm.ace.cell<-function(data, exposure, outcome) {
  formula.cell<-as.formula(paste(outcome, "~", exposure,"+ age_change + AGE_correct + GENDER + fa_edu_3c + ma_edu_3c + NK + B + MO + GR + CD4 + CD8 + age_change:", exposure))
...
glm.ace.nd<-function(data, exposure, outcome) {
  ...
  formula_1<-as.formula(paste(outcome, "~", exposure,"+ age_change + AGE_correct + GENDER + fa_edu_3c + ma_edu_3c + STEA16 + age_change:", exposure))
  model.1<-lme(formula_1, 
               data = data,
               random = ~1+age_change|ID,
               weights = varFixed(~WEIGHT_NORM_OVERALL_EPIGEN),
...
glm.ace.ac<-function(data, exposure, outcome) {
  formula.ac<-as.formula(paste(outcome, "~", exposure,"+ age_change + AGE_correct + GENDER + fa_edu_3c + ma_edu_3c + EV1 + EV2 + EV3 + EV4 + EV5 + age_change:", exposure))
...
```{r}
sol.ana.long.cca<-sol.ana.long[complete.cases(sol.ana.long[,c("ACE_TOT","fa_edu_3c","ma_edu_3c","GENDER","AGE_correct")]),]
