...
#### Covariates
  ...
  - Second set (further adjusted in Model 2)
    ...
    - Hispanic/Latino background (note: I did not adjust it in the actual analyses since it had seven categories, which may decrease the power if it is not something we have to adjust)
...
### Longitudinal analysis setup
  ...
  - Generalized estimating equations (GEE) vs. Generalized linear mixed-effect models (GLMM)
  
  - Each model has been account for the sampling weight for each subject
...
### Primary analysis
  ...
  - We can see the results from GEE and GLMM did not have a very good consistency
  
    - Directions were the same
    
    - However, the difference in the magnitude was not negligble
  
  - From my perspective, the unbalanced design in our study is the most probable attribute to this situation (discuss in the sensitivity part)
  
  - GLMM also seemed to have large power than GMM
...
kable(test, format = "latex", booktabs = T, digits = 3, caption = "Association of Two-level ACEs and Total ACEs Score with EAA indictors")%>% add_header_above(c(" " = 1,"GEE" = 4, "GLM" = 4))%>% pack_rows(index = c("GrimAA"=12, "GrimAge"=12,"DunedinPACE"=12))%>%
...
#### Adjusted for cell types
  ...
  - Adjusted for cell types seemed increased the power
  
  - Probably because cell types were associated with the outcome but not the exposure
...
glm.ace.cell<-function(data, exposure, outcome) {
  formula.cell<-as.formula(paste(outcome, "~", exposure,"+ age_change + AGE + GENDER + CENTER + EDUCATION_C2 + age_arr_US + NK + B + MO + GR + CD4 + CD8 + age_change:", exposure))
...
kable(test.1, format = "latex", booktabs = T, digits = 3, caption = "Sensitivity Analysis by Adjusting for Cell Types")%>% add_header_above(c(" " = 1, "GLM" = 4))%>% pack_rows(index = c("GrimAA"=6, "GrimAge"=6,"DunedinPACE"=6))%>%
  pack_rows(index = rep(c("Model 2 + cell types" = 3),6))
```

#### Using three-level ACEs

  - When changing the reference group from "<4 ACEs" to "No ACE", the association was better revealed
  
  - There is evidence that even small number of ACEs can associated with EAA
...
#### Unbalanced desgin
  ...
  - Also, there are some outliers with unreasonable interval between two visit
  ...
  - The GEE cannot handle the unbalanced design in the longitudinal study
  
    - So only GLMM was used here
    
  - The results were consistent to the primary analysis using GLMM but not GEE
  
  - Maybe think about should we recognize the nature of unbalanced desgin in our study, or consider it the balanced design with 6-year interval between two visits.
...
# Subjects that had the interval <5 or >9
...
# Exclude the outlier
sol.ana.long.ub<-sol.ana.long%>%filter(!ID%in%outlier_ID)
```
...
kable(test.3, format = "latex", booktabs = T, digits = 3, caption = "Sensitivity Analysis by Accounting for Unbalanced Design")%>% add_header_above(c(" " = 1, "GLM" = 4))%>% pack_rows(index = c("GrimAA"=12, "GrimAge"=12,"DunedinPACE"=12))%>%
  pack_rows(index = rep(c("Model 1" = 3, "Model 2" = 3),6))
```
...
#### Adjust for ancestry
  ...
  - The results did not apparently shift after the adjustment
...
kable(test.4, format = "latex", booktabs = T, digits = 3, caption = "Sensitivity Analysis by Adjusting for Ancestry")%>% add_header_above(c(" " = 1, "GEE" = 4, "GLM" = 4))%>% pack_rows(index = c("GrimAA"=6, "GrimAge"=6,"DunedinPACE"=6))%>%
