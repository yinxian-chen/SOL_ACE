...
### Primary analysis
  ...
  - Both 4+ ACEs and ACEs score is positively associated with EAA at first visit (hold for all EAA indicators)
    ...
    - 4+ ACEs
      ...
      - GrimAge: `r round(result.matrix$beta[4],2)` (`r round(result.matrix$lower[4],2)`, `r round(result.matrix$upper[4],2)`)
...
#### Parental separate as the covariate
  ...
  - Pulling out parental separation showed consistent results
...
#### Using three-level ACEs
  ...
  - 4+ ACEs was associated with increased EAA at the first visit compared with none
    ...
    - GrimAge: `r round(result.matrix.2$beta[7],2)` (`r round(result.matrix.2$lower[7],2)`, `r round(result.matrix.2$upper[7],2)`)
      
    - DunedinPACE: `r round(result.matrix.2$beta[12],2)` (`r round(result.matrix.2$lower[12],2)`, `r round(result.matrix.2$upper[12],2)`)
...
#### Assessment for ACE items
  ...
  - **Physical abuse** + **Sexual Abuse** + **Emotional Neglect** + **Parental separation** + **Witness intimate partner violence** + **Substance abuse in household members** + **Mental illness in household members** + **Having household members in prison**
...
#### Stratified analysis by birth place
  ...
  - Strong evidence showed that birth place modified the association of ACEs and EAA at the first visit
