...
```{r, echo=F}
table.1.eaa<-sol.ana.long%>%group_by(time)%>%mutate(time_cca = ifelse(is.na(eaa_grim),NA,time))%>%ungroup()%>%
  select(age_t,GENDER,ma_edu_3c,fa_edu_3c,age_arr_US,US_BORN, CIGARETTE_USE,ACE_TOT,ace_c3, time_cca)%>%
      ...
      ~ .x %>%
        tbl_summary(
          ...
          type = list(c(age_t,ACE_TOT) ~ "continuous", 
              c(GENDER,ma_edu_3c,fa_edu_3c,age_arr_US,US_BORN,CIGARETTE_USE,ace_c3) ~ "categorical"),
          value = NULL,
          missing = "no",
...
ana.plot.1<- ggplot(data=result.matrix.1,
  ...
  geom_hline(yintercept =0, linetype=2)+
  ylab("Coefficient (95% CI)")+
  labs(title = "Association between ACE Score and EAA over Average Six-year Follow-up", col = "Variables")+
  geom_errorbar(aes(ymin=lower, ymax=upper,col=variable),width=0.2,cex=0.5) +
  facet_wrap(~outcome,strip.position="left",nrow=3,scales = "free_y")+theme_bw()+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
ana.plot.specific.1+ theme(plot.title = element_text(size = 10.5))
```
...
#### Adjust for ancestry
  ...
  - Only **778** subjects had the ancestry information
...
```{r,include=F}
sol.ana.long.cca<-sol.ana.long[complete.cases(sol.ana.long[,c("ACE_TOT","fa_edu_3c","ma_edu_3c","GENDER")]),]
...
ana.plot.gender.1+ theme(plot.title = element_text(size = 10.5))
```
...
ana.plot.usb.1<- ggplot(data=result.matrix.usb.1,
  ...
  geom_hline(yintercept =0, linetype=2)+
  ylab("Coefficient (95% CI)")+
  labs(title = "Association between ACE Score and EAA over Average Six-year Follow-up by Nativity Status", col = "Variables", shape = "Nativity status")+
  geom_errorbar(aes(ymin=lower, ymax=upper,col=variable),width=0.2,cex=0.5) +
  facet_wrap(~outcome,strip.position="left",nrow=3,scales = "free_y")+theme_bw()+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
ana.plot.usb.1+ theme(plot.title = element_text(size = 9.5))
```
...
#### Account for time at arrival
  ...
  - Did not change the conclusion that nativity status modified the association
  
  - Named as **"Immigration status"**
...
#### Check if smoking fully explained the association across immigration status
